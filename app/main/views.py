...
def index():                                                                  
    ...
    # Get available counties for display
    counties = County.query.filter_by(active=True).all()
    return render_template('main/index.html', counties=counties) 
...
.order_by(PermitApplication.submitted_at.desc()).all()                
    ...
    if current_user.county_id:                                                
        permit_types = PermitType.query.join(Department).filter(Department.county_id == current_user.county_id, PermitType.active ).limit(6).all()                                                   
